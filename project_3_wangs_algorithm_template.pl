 ...
 */
 ...
 /*
  Implication/right (non-branching)
  A -> B on the right => move A to the left, keep B on the right
  E.g.: L => A -> B, R becomes A, L => B, R  
 */
...
% Write your code here
/* 
  Disjunction/left (branching)
  A v B on the left => two branches, each with one disjunct:
  Branch 1: A, rest-of-left => R
  Branch 2: B, rest-of-left => R
 */
prove(L => R) :-
  member(A v B, L),
  del(A v B, L, NewL),
...
/* 
  Conjunction/right (branching)
  A ^ B on the right => two branches, each with one conjunct:
  Branch 1: L => A, rest-of-right
  Branch 2: L => B, rest-of-right
 */
prove(L => R) :- 
  member(A ^ B, R),
  del(A ^ B, R, NewR),
